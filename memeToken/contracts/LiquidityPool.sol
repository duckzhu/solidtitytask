...
//这个项目的流动池只收交易税，没有其他费用，且是固定比例的交易税，流动池来源于管理员添加的流动性，这其实有中心化的风险，
// 会引发信用问题，容易rug pull。最好是由社区来维护流动性池，管理员只是一个监督者的角色，管理员可以设置交易税比例和交易税地址
    struct LPAmount {
        // 流动池ETH持有量
        uint ethLPReserve;
        // 流动池MEME持有量
    ...
    // 购买MEME，实际上是把ETH卖给流动性池，流动性池根据x*y=k算法计算出能买到多少MEME，
    // 再扣除交易税,剩余的MEME发送给买家
    // 购买限制见后面tradeValidate
    ...
    // 卖出MEME，实际上是把MEME卖给流动性池，流动性池根据x*y=k算法计算出能卖出多少ETH，
    // 再扣除交易税,剩余的ETH发送给卖家，交易税销毁或者分配给税收地址（销毁造成稀缺性，增加价值，目前采取这个方式）
    // 卖出限制见后面tradeValidate
    ...
    //以下是购买限制校验逻辑
    // 单次交易最大10个ETH  避免大额交易对流动性池的冲击
    // 每次交易间隔必须大于10分钟或者每个用户每天最多交易10次， 避免频繁交易
